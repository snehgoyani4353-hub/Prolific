...
<head>
  ...
  <title>PROLIFIC | ${emp.name} — Employee Workspace Portal</title>
  <meta name="description" content="Prolific Enterprise Employee Workspace Portal for ${emp.name} (${emp.code}) — Architectural drafting, task execution, and drawing submissions.">
  ...
  <header class="wf-header">
    ...
    <div class="wf-header-right">
      ...
      <div class="user-profile-badge">
        <span id="empHeaderUserName">${emp.name} (${emp.code})</span>
      </div>
    </div>
  </header>
    ...
    <aside class="wf-sidebar">
      ...
      <div class="personnel-category">
        <div class="cat-header">
          <span>Active Profile (<span id="empSidebarNameCode">${emp.name} • ${emp.code}</span>)</span>
        </div>
      ...
      <div style="margin-top: 24px; padding: 14px; background: #FAF8F4; border: 1.5px solid #E2D9C8; border-radius: 10px; font-size: 0.78rem; color: #64748B;">
        ...
        <div style="font-weight: 700; color: #1E2532; font-size: 0.88rem;">📍 ${emp.city} Regional Hub</div>
        <div style="margin-top: 4px;">Role: <strong>${emp.work}</strong></div>
      </div>
    </aside>
      ...
      <section class="wf-card-panel" id="empPendingSection">
        <div class="wf-card-header" style="display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 12px;">
          ...
          <span style="font-size: 0.85rem; color: #64748B; font-weight: 600;">
            <span id="empWorkflowSubtext">${emp.name}'s</span> active workflow. Direct execution mode.
          </span>
        </div>
          ...
          <div class="emp-sub-card">
            <div>
              <div style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 10px; flex-wrap: wrap; gap: 8px;">
                <h3 id="cardTaskTitle" style="font-family: var(--font-serif); font-size: 1.15rem; font-weight: 800; color: #1E2532; margin: 0;">Drawing for ${emp.city}</h3>
                <span class="luxury-city-pill">📍 <strong id="cardTaskTarget">${emp.city}</strong></span>
              </div>
...
employees.forEach(emp => {
  const filename = emp.name.toLowerCase().replace(/[^a-z0-9]+/g, '-') + '.html';
  const filePath = path.join(__dirname, filename);
  fs.writeFileSync(filePath, generateHTML(emp), 'utf-8');
  console.log(`Generated: ${filename}`);
});

// Also generate employee-dashboard.html (default to Komal)
fs.writeFileSync(path.join(__dirname, 'employee-dashboard.html'), generateHTML({
  name: 'Komal',
  code: 'EMP012',
  work: 'Drawing',
  city: 'Ahmedabad'
}), 'utf-8');
console.log('Generated: employee-dashboard.html');
